...
class LiteralValuePacket : public Packet {
  ...
  public:
    ...
    static LiteralValuePacket* ParseLiteralValuePacket(const vector<bool>& input);
};

LiteralValuePacket* LiteralValuePacket::ParseLiteralValuePacket(const vector<bool>& input) {
  // TODO consider making the return value any unused elements from the end of input
  // TODO use new to create the returned pointers
  return NULL;
}
...
class OperatorPacket : public Packet {
  ...
  public:
    ...
    void AppendContainedPacket(Packet* p) { this->contained_packets.push_back(p); } // TODO remove this if my code can instead set all the contained packets in the object constructor
...
// ParseOperatorPacket will iterate over the bytes pointed to between the given iterators (most significant bit first) and return the operator packet
  ...
  const vector<bool>::const_iterator& end)
{
  ...
  cout << "Version: " << version << ", Type: " << type << endl;
  ...
  // Move through any remaining zero-padding before returning this packet and remaining bits, if any
  // TODO HERE
  // TODO update current to the next bit that we haven't processed
...
pair<Packet*,vector<bool>::const_iterator> ParseSinglePacket(vector<bool>::const_iterator start, const vector<bool>::const_iterator& limit) {
  ...
  Packet* p = NULL;
  vector<bool>::const_iterator remaining;
  if (type_id == literal_val) {
    // TODO NEXT 20
  }
  else {
    OperatorPacket::ParseOperatorPacket(start, limit);
    // TODO NEXT 5
  }

  return pair<Packet*,vector<bool>::const_iterator>(p, limit);
}

unsigned int SumAllPacketVersions(const Packet* packet) {
  // TODO NEXT 10 recursively call this on all contained packets - consider making it an object method on top of operator packet which will recursively call it for all of its contained packets
  return 0;
}
...
int main() {
  ...
  if (packet_result.second != input.end()) {
  ...
  unsigned int version_total = SumAllPacketVersions(outer_packet);
  cout << "Part 1 answer: " << version_total << endl;

  // TODO move my notes to comments over functions, etc.
  // The entire sequence is a single packet which contains many other packets
  // Disregard any extra 0 bits at the end of the outermost packet

  // Packet header: 3 bits version, 3 bits type ID (both binary numbers, i.e. 100=4)
  // Type ID 4: literal value; the rest of the packet encodes a single binary number, padded with leading zeroes until its length is a multiple of four bits; each group of four bits is prefixed by a 1 bit except the last group, which is prefixed by a 0 bit; ex. 110 (version) 100 (type ID) 10111 (0111 is the first bits of the value) 11110 (1110 is the next bits of the value) 00101 (0101 is the last bits of the value) 000 (just padding zeroes)

  

  // helper function to decode a literal value from bits, and return any unused bits after the 0-padding
  // helper function to decode an operator value from bits, which may recursively call itself or call the literal-decoder; they can return objects representing a value or an operator, which can each contain a vector of Packets inside them
